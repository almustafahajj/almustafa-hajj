...
## (اختياري) تفعيل «📷 قراءة جواز» على الويب
الميزة تعمل بدونها البرنامج طبيعياً (تظهر رسالة لطيفة تقترح الإدخال اليدوي أو
استيراد إكسل). لتفعيلها على PythonAnywhere:
```bash
source ~/venv/bin/activate
pip install -r requirements-web-ocr.txt
```
ثم **Reload**. محرّك `tesseract` وحزمة اللغة العربية `ara` مثبّتان مسبقاً على
نظام PythonAnywhere، فلا حاجة لتثبيتهما. تنبيه: القراءة تستهلك من المعالج
والقرص المحدودين في النسخة المجانية — إن ظهرت مشاكل، الأسرع أن تُقرأ الجوازات
على سطح المكتب ثم تُصدَّر إكسل وتُستورد هنا.
