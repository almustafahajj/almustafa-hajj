...
def export_rooming_cards_pdf(records: list, path: str | Path, *,
    ...
    def card(number, typ, occ):
        ...
        for i, r in enumerate(occ, 1):
            vals = [str(i), r.full_name_ar or r.full_name_en or "",
                    str(r.family_number or ""),
                    str(getattr(r, "transport", "") or "")]
            data.append(_ar_cells(list(reversed(vals)), st["cell"], avail))
    ...
    doc.build(story,
              ...
              onLaterPages=lambda c, d: _footer_portrait(c, d, "تسكين الغرف"))
    return path
...
def build_quotation_data(rec, *, trip=None, company=None, number: str = "",
    ...
    transport_lines = [[dep, _qtr("مطار جدة", "locations", en),
                        _qtr("الفندق في مكة", "locations", en)],
                       ["", _qtr("الفندق في مكة", "locations", en),
                        _qtr("الفندق في المدينة", "locations", en)],
                       [ret, _qtr("الفندق في المدينة", "locations", en),
...
def umrah_quotation_schema(lang: str = "ar") -> list:
    """مخطّط حقول عرض السعر بلغة النموذج (ar/en): التسميات والعناوين والقوائم
    المنسدلة كلها تُترجم، فيظهر الطلب كاملاً بالإنجليزية عند اختيار EN."""
    en = str(lang) == "en"

    def L(a, e):                                   # تسمية/عنوان مترجَم
        return e if en else a

    def O(a_list, e_list):                         # خيارات قائمة منسدلة مترجَمة
    ...
    title_opts = O(["عرض سعر رحلة عمرة", "عرض سعر برنامج عمرة",
                   ...
                   ["Umrah Trip Quotation", "Umrah Program Quotation",
                    "Hajj Program Quotation", "Hotel Stay Quotation"])
    ...
    return [
        {"legend": L("بيانات العرض", "Offer Details"), "fields": [
            {"key": "number", "label": L("الرقم المرجعي", "Reference No."),
             "ro": True},
            {"key": "date", "label": L("التاريخ", "Date"), "type": "date"},
            {"key": "lang", "label": L("لغة العرض", "Offer Language"),
             "type": "select", "options": ["ar", "en"]},
            {"key": "title", "label": L("عنوان العرض", "Offer Title"),
             "type": "combo", "options": title_opts},
            {"key": "greeting", "label": L("عبارة التحية", "Greeting"),
             "type": "combo", "options": greet_opts},
            {"key": "addressed_title", "label": L("لقب المستلِم", "Title"),
             "type": "select", "options": addr_t},
            {"key": "addressed_to",
             "label": L("عناية (اسم المستلِم)", "Attention (Recipient)")},
            {"key": "period_from", "label": L("الفترة من", "Period From"),
             "type": "date"},
            {"key": "period_to", "label": L("الفترة إلى", "Period To"),
             "type": "date"},
        ]},
        {"legend": L("الضيوف", "Guests"), "table": "guests", "columns": [
            {"key": "count", "label": L("العدد", "Count"), "type": "select",
             "options": NUM},
            {"key": "type", "label": L("النوع", "Type"), "type": "select",
             "options": guest_t}]},
        {"legend": L("الإقامة", "Accommodation"), "table": "stays", "columns": [
            {"key": "city", "label": L("المدينة", "City"), "type": "select",
             "options": cities},
            {"key": "nights", "label": L("الليالي", "Nights"), "type": "select",
             "options": NIGHTS},
            {"key": "hotel", "label": L("الفندق", "Hotel"), "type": "select",
             "options": hotels},
            {"key": "room", "label": L("نوع الغرفة", "Room Type"),
             "type": "select", "options": rooms},
            {"key": "rooms", "label": L("عدد الغرف", "Rooms"), "type": "select",
             "options": NUM},
            {"key": "view", "label": L("الإطلالة", "View"), "type": "select",
             "options": views},
            {"key": "meals", "label": L("الوجبات", "Meals"), "type": "select",
             "options": meals},
            {"key": "cin", "label": L("الدخول", "Check-in"), "type": "date"},
            {"key": "cout", "label": L("المغادرة", "Check-out"),
             "type": "date"}]},
        {"legend": L("الطيران", "Flights"), "fields": [
            {"key": "flight_class", "label": L("درجة الطيران", "Flight Class"),
             "type": "select", "options": fclass}]},
        {"legend": L("جدول الطيران", "Flight Schedule"), "table": "flights",
         "columns": [
            {"key": "day", "label": L("التاريخ", "Date"), "type": "date"},
            {"key": "carrier", "label": L("الناقل", "Carrier"), "type": "select",
             "options": carriers},
            {"key": "depart", "label": L("الإقلاع", "Departure")},
            {"key": "from", "label": L("من", "From"), "type": "select",
             "options": airports},
            {"key": "arrive", "label": L("الوصول", "Arrival")},
            {"key": "to", "label": L("إلى", "To"), "type": "select",
             "options": airports}]},
        {"legend": L("المواصلات", "Transportation"), "fields": [
            {"key": "car_type", "label": L("نوع السيارة", "Car Type"),
             "type": "select", "options": cars},
            {"key": "car_model", "label": L("الموديل", "Model"), "type": "select",
             "options": car_model},
            {"key": "car_count", "label": L("العدد", "Count"), "type": "select",
             "options": NUM}]},
        {"legend": L("بنود التنقّل", "Transfers"), "table": "transport_lines",
         "columns": [
            {"key": "date", "label": L("التاريخ", "Date"), "type": "date"},
            {"key": "from", "label": L("من", "From"), "type": "select",
             "options": transfer},
            {"key": "to", "label": L("إلى", "To"), "type": "select",
             "options": transfer}]},
        {"legend": L("قطار الحرمين (اختياري)", "Haramain Train (optional)"),
         "table": "trains", "columns": [
            {"key": "tickets", "label": L("التذاكر", "Tickets"),
             "type": "select", "options": NUM},
            {"key": "class", "label": L("الدرجة", "Class"), "type": "select",
             "options": train_cls},
            {"key": "from", "label": L("من", "From"), "type": "select",
             "options": train_stn},
            {"key": "to", "label": L("إلى", "To"), "type": "select",
            ...
            {"key": "depart", "label": L("الإقلاع", "Departure")},
            {"key": "arrive", "label": L("الوصول", "Arrival")}]},
        {"legend": L("التأشيرات", "Visas"), "fields": [
            {"key": "visa_count", "label": L("العدد", "Count"), "type": "select",
             "options": NUM},
            {"key": "visa_type", "label": L("النوع", "Type"), "type": "select",
             "options": visa}]},
        {"legend": L("التسعير", "Pricing"), "table": "pricing", "columns": [
        ...
        {"legend": L("الصلاحية والملاحظات والختام",
                     "Validity, Notes & Closing"), "fields": [
            {"key": "currency", "label": L("العملة", "Currency"),
             "type": "select", "options": currency},
            {"key": "validity", "label": L("صلاحية العرض حتى", "Valid Until"),
             "type": "date"},
            {"key": "validity_time",
             "label": L("وقت انتهاء الصلاحية", "Expiry Time"), "type": "select",
             "options": times},
            {"key": "note", "label": L("ملاحظة", "Note"), "type": "area"},
            {"key": "closing", "label": L("عبارة الختام", "Closing"),
             "type": "area"}]},
        {"legend": L("إظهار / إخفاء بنود العرض", "Show / Hide Sections"),
         "fields": [
            {"key": "show_stays",
             "label": L("إظهار الإقامة", "Show Accommodation"), "type": "bool"},
            {"key": "show_flights", "label": L("إظهار الطيران", "Show Flights"),
             "type": "bool"},
            {"key": "show_transport",
             "label": L("إظهار المواصلات", "Show Transport"), "type": "bool"},
            {"key": "show_costs", "label": L("إظهار التكلفة", "Show Cost"),
             "type": "bool"}]},
        {"legend": L("التوقيع (المدير العام)", "Signature (General Manager)"),
         "fields": [
            {"key": "gm_title", "label": L("الصفة", "Role"), "type": "select",
             "options": gm_title},
...
def _looks_ltr(text: str) -> bool:
    """نصّ لاتيني (بلا حروف عربية) — لعرض أسماء الفنادق الإنجليزية كما هي."""
...
def _draw_card_bg(c, reader, bw, bh):
    ...
    if reader is not None:
        try:
            c.drawImage(reader, 0, 0, bw, bh, mask="auto",
                        preserveAspectRatio=False)
            return
        except Exception:                              # noqa: BLE001
            pass
    c.saveState()                                      # بديل: شريط أسود بخطّ ذهبي
    w = _CARD_PATTERN_FRAC * bw
    c.setFillColor(colors.HexColor("#111111"))
    c.rect(0, 0, w, bh, fill=1, stroke=0)
    c.setStrokeColor(_ACCENT)
    c.setLineWidth(1.4)
    c.line(w - 0.7, 0, w - 0.7, bh)
    c.restoreState()


def _draw_social_icons(c, cx, cy, r, gap):
    """أربع أيقونات دائرية ذهبية: فيسبوك · إنستغرام · يوتيوب · تيك توك."""
    gold = _ACCENT
    white = colors.white
    xs = [cx + (i - 1.5) * gap for i in range(4)]
    for x in xs:                                       # الدوائر الذهبية
        c.setFillColor(gold)
        c.circle(x, cy, r, fill=1, stroke=0)
    # فيسبوك: حرف f أبيض
    c.setFillColor(white)
    c.setFont(_FONT_BOLD, r * 1.7)
    c.drawCentredString(xs[0], cy - r * 0.62, "f")
    # إنستغرام: مربّع بحواف + عدسة + نقطة
    x = xs[1]
    c.setStrokeColor(white)
    c.setLineWidth(max(0.5, r * 0.16))
    c.roundRect(x - r * 0.55, cy - r * 0.55, r * 1.1, r * 1.1, r * 0.32,
                fill=0, stroke=1)
    c.circle(x, cy, r * 0.32, fill=0, stroke=1)
    c.setFillColor(white)
    c.circle(x + r * 0.34, cy + r * 0.34, r * 0.1, fill=1, stroke=0)
    # يوتيوب: مستطيل أبيض + مثلّث تشغيل ذهبي
    x = xs[2]
    c.setFillColor(white)
    c.roundRect(x - r * 0.62, cy - r * 0.42, r * 1.24, r * 0.84, r * 0.22,
                fill=1, stroke=0)
    c.setFillColor(gold)
    tri = c.beginPath()
    tri.moveTo(x - r * 0.16, cy + r * 0.26)
    tri.lineTo(x - r * 0.16, cy - r * 0.26)
    tri.lineTo(x + r * 0.30, cy)
    tri.close()
    c.drawPath(tri, fill=1, stroke=0)
    # تيك توك: نوتة موسيقية بيضاء
    x = xs[3]
    c.setFillColor(white)
    c.ellipse(x - r * 0.5, cy - r * 0.55, x - r * 0.06, cy - r * 0.16,
              fill=1, stroke=0)
    c.setLineWidth(max(0.6, r * 0.16))
    c.setStrokeColor(white)
    c.line(x - r * 0.08, cy - r * 0.35, x - r * 0.08, cy + r * 0.5)
    p = c.beginPath()                                  # العلَم
    p.moveTo(x - r * 0.08, cy + r * 0.5)
    p.curveTo(x + r * 0.28, cy + r * 0.42, x + r * 0.32, cy + r * 0.14,
              x + r * 0.16, cy + r * 0.02)
    c.drawPath(p, fill=0, stroke=1)
...
def export_badges_pdf(records: list, path: str | Path, *,
    ...
    التصميم: نقش جانبي على اليسار، شعار المصطفى وأيقونات التواصل أعلى الوسط،
    ثم الصورة (الشخصية للرجال من صورة الجواز، وصورة المرأة الثابتة للنساء)،
    فالاسم و«فندق / …»، وشعار نيرفانا في الأسفل.
    الخلفية: طبيب/واعظ الحملة + الإداريون + رقم الطوارئ.
    """
    ...
    def center(text, y, font, size, color=_INK):
        c.setFillColor(color)
        c.setFont(font, size)
        c.drawCentredString(cx, y, ar(str(text)))
    ...
    def draw_img_centered(reader, top_y, max_w, max_h):
        """يرسم صورة (شعار) متمركزة أفقياً بأعلاها عند top_y، ويعيد أسفلها."""
        if reader is None:
            return top_y
        ...
        if h > max_h:
            h = max_h
            w = h * iw / ih
        c.drawImage(reader, cx - w / 2, top_y - h, w, h,
                    mask="auto", preserveAspectRatio=True)
        return top_y - h
    ...
    for i, rec in enumerate(records):                  # صفحات الوجوه
        if i and i % PER == 0:
            c.showPage()
        in_cell(i % PER, lambda r=rec: draw_front(r))
    c.showPage()

    for idx in range(PER):                             # ورقة خلفية واحدة موحّدة
