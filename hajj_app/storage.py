...
# ----------------------------------------------- تسكين المخيمات (مشفّر، لكل وضع)
def camps_path() -> Path:
    """مسار حفظ تسكين المخيمات — مشفّر ومستقلّ لكل وضع (حج/عمرة)."""
    base = default_data_path()
    return base.with_name("camps_" + base.stem + ".json")


def load_camps(session: Session | None = None) -> dict:
    """يحمّل تسكين المخيمات المحفوظ (يفكّ التشفير بالجلسة). يعيد {} عند التعذّر."""
    path = camps_path()
    if not path.is_file():
        return {}
    try:
...
def save_camps(camps: dict, session: Session | None = None) -> None:
    """يحفظ تسكين المخيمات (مشفّراً إن وُجدت جلسة) — كتابة ذرّية."""
    path = camps_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    text = json.dumps(camps, ensure_ascii=False).encode("utf-8")
    blob = (_ENCRYPTED_MAGIC + session.encrypt(text)) if session else text
    temp = path.with_suffix(".tmp")
    with open(temp, "wb") as fh:
        fh.write(blob)
